...
$(document).on('click', 'a.actName', function() {
    ...
    success: function(data) {
      if (data.code == '001') {
        $.each(data.result, function(indexInArray, content) {
          ...
          '-link collapsed actName" href="javascript:;" id="' +
          content.act_Id + '">' +
    ...
    success: function(data) {
      if (data.code == '001') {
        $.each(data.result, function(index, content) {
          ...
          '<a class="nav-link collapsed actName" href="javascript:;"' +
           ' id="' + content.act_Id + '">' +
          '<i class="fas fa-fw fa-bullhorn"></i>' +
          '<span class="all-act-title">' + content.act_Name +
          '</span></a></li>';
...
function backToIndex() {
  ...
  if ($('a.actName').length >= 1) {
    if ($.cookie('actid') == undefined) {
      $.cookie('actid', $('a.actName').eq(0).prop('id'), {path: '/'});
    }
